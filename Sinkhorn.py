...
def softmin_torch_image(h, C1, C2, eps):
    ...
    # (B, N1, 1, N2)
    h = h.view(B, N1, 1, N2).contiguous()
    h = torch.logsumexp(h - C2[:, None, :, :]/eps,
                        dim=3, keepdims=True)     # (B, N1, M2, 1)
    # (B, M2, 1, N1)
    h = h.permute((0, 2, 3, 1)).contiguous()
    h = torch.logsumexp(h - C1[:, None, :, :]/eps,
                        dim=3, keepdims=True)     # (B, M2, M1, 1)
    h = h.permute((0, 2, 1, 3)).reshape(
        B, M1, M2).contiguous()               # (B, M1, M2)
    return h
...
def softmin_keops_image(h, xs, ys, eps):
    ...
    # (B*N1, 1, N2)
    h = h.reshape(B*N1, 1, N2).contiguous()
    # (B*N1, M2)
    h = softmin_keops_line(h, x2, y2, eps)
    h = h.reshape((B, N1, M2)).permute(
        (0, 2, 1)).contiguous().reshape((B*M2, 1, N1))  # (B*M2, 1, N1)
    # (B*M2, M1)
    h = softmin_keops_line(h, x1, y1, eps)
    h = h.reshape((B, M2, M1)).permute(
        (0, 2, 1)).contiguous()                        # (B, M1, M2)
    return h
...
def softmin_cuda_image(h, Ms, Ns, eps, dx):
    ...
    # (B*N1, N2)
    h = h.view(B*N1, N2).contiguous()
    # (B*N1, M2)
    h = LogSumExpCUDA(h, M2, dx_eff)
    h = h.reshape((B, N1, M2)).permute(
        (0, 2, 1)).contiguous().reshape((B*M2, N1))    # (B*M2, N1)
    # (B*M2, M1)
    h = LogSumExpCUDA(h, M1, dx_eff)
    h = h.reshape((B, M2, M1)).permute(
        (0, 2, 1)).contiguous()                        # (B, M1, M2)
    return h
...
class AbstractSinkhorn:
    ...
    Attributes
    ----------
    mu : torch.Tensor of size (B, M) or (B, M1, ...., Md)
    ...
    muref : torch.Tensor with same dimensions as mu 
            (except axis 0, which can have len = 1)
        First reference measure for the Gibbs energy, i.e. 
            K = muref \otimes nuref exp(-C/eps)
    nuref : torch.Tensor with same dimensions as nu 
            (except axis 0, which can have len = 1)
        Second reference measure for the Gibbs energy, i.e. 
            K = muref \otimes nuref exp(-C/eps)
...
class LogSinkhornTorchImage(AbstractSinkhorn):
    ...
    nu : torch.Tensor of size (B, N1, N2)
        Second marginals 
    C : tuple of torch.Tensor of size (M1, N1) and (M2, N2)
    ...
    def __init__(self, mu, nu, C, eps, **kwargs):
        ...
        if len(C[0].shape) == 2:
            for Ci in C:
                assert len(
                    Ci.shape) == 2, "Dimensions of costs accross dimensions are not consistent"
            C = tuple(Ci[None, :, :] for Ci in C)
        for (i, Ci) in enumerate(C):
            assert Ci.shape[1:] == (
                mu.shape[i+1], nu.shape[i+1]), "Dimensions of cost and marginal not matching"
    ...
    def get_new_alpha(self):
        ...
        return - self.eps * (
            softmin_torch_image(
                h, self.C[0], self.C[1], self.eps) + self.logmuref - self.logmu
        )
    ...
    def get_new_beta(self):
        ...
        return - self.eps * (
            softmin_torch_image(
                h, self.CT[0], self.CT[1], self.eps) + self.lognuref - self.lognu
        )
...
class LogSinkhornKeops(AbstractSinkhorn):
    ...
    def __init__(self, mu, nu, C, eps, **kwargs):
        ...
        # Check shapes
        assert len(
            x.shape) == 3, "x.shape must be of the form (B, M, dim) or (1, M, dim)"
        assert len(
            y.shape) == 3, "y.shape must be of the form (B, N, dim) or (1, N, dim)"
    ...
    def get_new_alpha(self):
        ...
        return - self.eps * (softmin_keops(h, x, y, self.eps) + self.logmuref - self.logmu)
...
class LogSinkhornKeopsImage(AbstractSinkhorn):
    ...
    def get_new_alpha(self):
        xs, ys = self.C
        h = self.beta / self.eps + self.lognu
        return - self.eps * (softmin_keops_image(h, xs, ys, self.eps) + self.logmuref - self.logmu)
...
class LogSinkhornCudaImage(AbstractSinkhorn):
    ...
    def get_new_alpha(self):
        dx, Ms, Ns = self.C
        h = self.beta / self.eps + self.lognu
        return - self.eps * (softmin_cuda_image(h, Ms, Ns, self.eps, dx) + self.logmuref - self.logmu)

    def get_new_beta(self):
        dx, Ms, Ns = self.C
        h = self.alpha / self.eps + self.logmu
        return - self.eps * (softmin_cuda_image(h, Ns, Ms, self.eps, dx) + self.lognuref - self.lognu)
